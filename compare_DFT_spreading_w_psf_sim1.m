%%%%%%%%%%%%%%%%%%%%%  观察脉冲成型滤波器的DFT扩频的PAPR分析  %%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%   compare_DFT_spreading_w_psf_sim1.m    %%%%%%%%%%%%%
...
clear, clf
N=256; % FFT size
Nd=64; % subcarriers per user (Data block size)
...
for i_b=1:length(bs)
   b=bs(i_b);
   M=2^b; % Number of bits per QAM symbol and Alphabet size
   rand('twister',5489); randn('state',0);
   CCDF_IF0 = CCDF_PAPR_DFTspreading_sim1('IF',Nd,b,N,dBcs,Nblk);
   subplot(210+i_b), semilogy(dBs,CCDF_IF0,'k'), hold on 
   CCDF_LF0 = CCDF_PAPR_DFTspreading_sim1('LF',Nd,b,N,dBcs,Nblk);
   semilogy(dBs,CCDF_LF0,'k'), hold on
   for i=1:length(rhos)
      rho = rhos(i); % Roll-off factor
      psf = rcosfir(rho,Nsym,Nos,1,'norm')*Nos;  % RC filter coeff
      CCDF_IF = CCDF_PAPR_DFTspreading_sim1('IF',Nd,b,N,dBcs,Nblk,psf,Nos);
      CCDF_LF = CCDF_PAPR_DFTspreading_sim1('LF',Nd,b,N,dBcs,Nblk,psf,Nos);
      semilogy(dBs,CCDF_IF,['-' gss(i)], dBs,CCDF_LF,[':' gss(i)])
      str1(i,:)=sprintf('IFDMA with a=%3.1f',rho);
      str2(i,:)=sprintf('LFDMA with a=%3.1f',rho);
   end 
   legend(str11,str12,str1(1,:),str2(1,:),str1(2,:),str2(2,:),str1(3,:),str2(3,:),str1(4,:),str2(4,:),str1(5,:),str2(5,:),str1(6,:),str2(6,:))
...
for i=1:N_Nds
   Nd=Nds(i); 
   rand('twister',5489); randn('state',0);
   CCDF_LFDMa = CCDF_PAPR_DFTspreading_sim1('LF',Nd,b,N,dBcs,Nblk,psf,Nos); % CCDF of LFDMA
